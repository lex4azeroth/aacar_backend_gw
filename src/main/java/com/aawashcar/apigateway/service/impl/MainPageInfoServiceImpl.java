...
public class MainPageInfoServiceImpl extends BaseService implements MainPageInfoService {
	...
	public MainPageInfo getMainPageInfo(String validId) {
		...
		ResponseEntity<VehicleCategory[]> vehicleCategoryResponseEntity =
		                restTemplate.getForEntity(url, VehicleCategory[].class);
		...
		ResponseEntity<WashCarService[]> washCareServiceResponseEntity =
		                restTemplate.getForEntity(url, WashCarService[].class);
		...
		if (user.getId() > 0) {
			...
			resiQuarter = restTemplate.getForObject(url, ResidentialQuarter.class);
			mainPageInfo = EntityMapper.buildDefaultMainPageInfo(vehicleCategories, vehicleTypes, services, city,
	...
	public List<ResidentialQuarterModel> listResidentialQuarters(int provinceId, int cityId, int districtId) {
		...
		ResponseEntity<ResidentialQuarter[]> residentialQuarterResponseEntity =
		                restTemplate.getForEntity(url, ResidentialQuarter[].class);
